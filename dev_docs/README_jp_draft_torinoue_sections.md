...
- **バッファリング:** `Connection` の `_recvBuffer` / `_sendBuffer` で `\r\n` 行単位に再構成
...
#### 2.4.2 層間の責務の分離
各層は[interface.md](dev_docs/interface.md)のAPIを参照し内部の実装については知らなくて良いという思想を採用
- QUIT,Timeout,disconnectにおいて、A層がIRCのプロトコルを知る必要性がうまれたため、A層B層の中間層としてLifecycle層を導入した
- C層では、Client,Channelの所属先・所属者を管理するために状態を同期的に管理するファサードを導入した
...
| Linux（校舎 Ubuntu） | ✅ `make release` / `make test` / nc 手動結合テスト済み |
...
### 3.6 bot
botはサーバとは別のクライアントとして、起動する。
これによりbotに問題がうまれてもサーバがクラッシュすることがない。
```bash
make bonus
./ircbot <host> <port> <password> <channel> [nickname]
```
...
課題書 Chapter III に従い、AI の使用は開示する。
...
| 単体・結合テスト | テストの生成 | テストがカバーすべき事項の指示　|
...
| 英語版 `README.md` への翻訳 | **翻訳**（予定）         | 内容確認（統合・最終レビュー）          |
