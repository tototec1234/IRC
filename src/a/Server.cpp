...
static void _handleShutdownSignal(int signo) {
  (void)signo;
...
Server::Server(int port, const std::string& pw)
    : _listenFd(-1), _state(pw), _healthMonitor(30) {
...
// 　 reinterpret_cast<struct sockaddr*>(&clntAddr) にしようか迷ったが、CPP06
// ex01 で危険だったので保留
/* **************************************************************************
// reinterpret_cast has almost no restrictions == very free == very dangerous!
Handle with care!
...
uintptr_t	Serializer::serialize(Data *ptr)
{
        ...
        // return(static_cast<uintptr_t>(ptr)); // Compile error: static_cast
cannot convert between pointer and integer types return
...
void Server::run() {
  ...
  while (!_isShutdownRequested()) {
    ...
    */
    ...
    if (tmp != ret) {
      tmp = ret;
      std::cout << BLUE_COLOR << "ret="
                << ret  // ret = ready な fd の総数（listen 限定ではない）
                << " fd=" << _pollfds[0].fd << " nfds=" << _pollfds.size()
                << " [listen] fd=" << _pollfds[0].fd
                << " revents="  // revents=0 → 今回の poll
                                // でイベントなし（監視は継続）
                << RESET_COLOR;
      ...
      for (size_t i = 0; i < _pollfds.size(); ++i) {
        std::cout << "  i=" << i << " fd=" << _pollfds[i].fd << " revents=";
    ...
    for (size_t i = 0; i < _pollfds.size(); ++i) {
      ...
      if (rev & POLLIN) {
        // 　①「クライアントの意思でclose() や
        // shutdown()を行った場合（TCPのFINパケット（EOF）が届くが、その場合でもPOLLIN（読み込み可能）フラグは立ったままである。この状態で_handleReadがrecv()を呼ぶと、ブロックせずに即座に0を返してくる
        // 　②PONGタイムアウトfdはループ末尾で処理
        if (!_handleRead(
                fd)) {  //  EOF/recvエラー/行長すぎ　は非自発的失敗として切断
          if (_connections.find(fd) != _connections.end())
            _notifyAndDisconnect(
                fd,
                "Connection reset");  // connection_lifecycle_integration.md　
                                      // 5.2 recv==0 / POLLHUP も
                                      // DisconnectEvent に寄せる場合　準拠
          --i;
          continue;
        }
      }
      // (3) 書ける状態なら送る  bircd: check_fd.c の if(revents & POLLOUT)
      // fct_write
      if (rev & POLLOUT) {
      ...
      /*
      POLLHUP は相手側が切断（hang up）した
      ことを示すイベントだが、受信バッファに未読データが残っていることがある。
      ...
      */
      if (rev & (POLLERR | POLLHUP | POLLNVAL)) {
        _notifyAndDisconnect(
            fd,
            "Connection reset");  // connection_lifecycle_integration.md　 5.2
                                  // recv==0 / POLLHUP も DisconnectEvent
                                  // に寄せる場合　準拠
    ...
    for (std::vector<int>::iterator it = timeOut.begin(); it != timeOut.end();
         ++it)
...
void Server::_acceptClient() {
  ...
  //	dispatch は NULL なら早期 return するが、未登録 fd を作らないため accept
  // 時に addClient しておく
  _state.addClient(cs, host);

#ifdef A_LAYER_DEBUG
  std::cout << GREEN_COLOR << "New client #"
            << cs  // csはクラアントのfd　#0 stdin  #1 stdout  #2 stderr #3
                   // listenソケット よって最初のクライアントは #4 から
            << " from " << host << ":" << ntohs(csin.sin_port) << RESET_COLOR
            << std::endl;
#endif
}
...
bool Server::_handleRead(int fd) {
  ...
  最初の短い行だけ pop して巨大な未完了行が _recvBuffer に残り続けます（次の
  recv が来ないと切断されない）
  */
  if (conn->isLineTooLong()) {  // pop する前に弾く
    std::cerr
        << RED_COLOR << "line too long (#" << fd << ") 長すぎ切断"
        << RESET_COLOR
        << std::endl;  // 切断理由を決めるのはA層の責任　とりま　デバッグ出力
    return false;      // run() が _notifyAndDisconnect を呼ぶ（QUIT 済みなら
                       // _connections ガードでスキップ）
  }
  ...
  while (conn->hasCompleteLine()) {
    ...
    CommandResult result =
        _dispatcher.dispatch(fd, msg, _state, _healthMonitor);
    applyCommandResult(
        result, fd);  // 送信先ごとに _enqueueReplies 経由で送信バッファへ積む
    if (result.shouldDisconnect) return false;
  }
  return true;
}

// bircd: client_read.c の close(cs); clean_fd(&e->fds[cs]); に対応。
//   bircd は clean_fd で FD_FREE マーク → 次ループ init_fd が pollfds
//   再構築でスキップ。 A層は _pollfds 持続なので close + _pollfds erase +
//   Connection delete を自分でやる。
void Server::_disconnectClient(int fd) {
  ...
  _healthMonitor.removeClient(
      fd);  // ④ライフサイクル層も除去（fd再利用バグ・リーク対策）
// _clients のエントリは updateActivity(fd) で作られ、_connections
// の有無とは独立。 従って_connections に無くても _clients には残り得るので、if
// の外（無条件）に置く
#ifdef A_LAYER_DEBUG
  std::cout << "client #" << fd << " gone away"
            << std::endl;  // bircd の gone away 相当
#endif
}
...
void Server::_closeAllFds() {
  while (!_connections.empty()) {
    int fd = _connections.begin()->first;
...
void Server::_enablePollout(int fd) {
  for (size_t i = 0; i < _pollfds.size(); ++i)
    /*　データを積んだとき → _enablePollout（init_fd.c L59-60
       の「strlen(buf_write)>0 で立てる」を、 積んだ瞬間に手動でやる）　*/
    if (_pollfds[i].fd == fd) {
...
void Server::_disablePollout(int fd) {
  for (size_t i = 0; i < _pollfds.size(); ++i)
    /*　送り切ったとき → _disablePollout（bircd
     * なら次ループで自動的に下りる部分を、手動で下ろす）*/
    if (_pollfds[i].fd == fd) {
      _pollfds[i].events &= ~POLLOUT;
      return;
    }  // disable　&= ~FLAG で下ろす
...
//   buf_write が空になれば自動で下りる。A は _pollfds
//   持続のため明示的に下ろす。
bool Server::_handleWrite(int fd) {
  Connection* conn = _connections[fd];
  if (!conn->writeToSocket()) return false;  // send 失敗 → 呼び出し側で切断
...
// 送信先 fd は source fd とは限らない（JOIN
// 等は他メンバーへブロードキャスト）。
void Server::applyCommandResult(const CommandResult& result, int sourceFd) {
  _enqueueReplies(result);
  if (result.shouldDisconnect) _notifyAndDisconnect(sourceFd, "Client Quit");
}
...
# 別ターミナル2枚:
        ...
        # \r\n無し200B → 切断の確認
        python3 -c "import sys; sys.stdout.buffer.write(b'A'*200)" | nc
localhost 6667
*/
