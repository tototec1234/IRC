...
> **用語:** 本 README の **Lesson** = bircd 学習段階。**Phase** = ircserv A 層実装フェーズ（[`a_implementation_plan.md`](../a_header_tmp/a_implementation_plan.md)）。対応表は [bircd_lesson_ircserv_phase_map.md](bircd_lesson_ircserv_phase_map.md)。
...
| 項目 | 内容 |
|------|------|
| **Lesson 1** | **必修。** select 版 bircd の読解を終えてから Lesson 2 以降に進む |
...
Lesson 1 を飛ばすと Lesson 2 の「なぜこう変えるか」が理解できません。
...
| [bircd_learning_curriculum.md](bircd_learning_curriculum.md) | Lesson 1〜5 の全カリキュラム |
| [bircd_learning_curriculum_ans.md](bircd_learning_curriculum_ans.md) | 解答・メモ（Lesson 2 以降の答え合わせはここへリンク） |
| [lesson2_guide.md](lesson2_guide.md) | **Lesson 2 ハンズオン**（select → poll） |
| [bircd_analysis.md](bircd_analysis.md) | bircd 構造の分析メモ |
| [bircd_lesson_ircserv_phase_map.md](bircd_lesson_ircserv_phase_map.md) | bircd Lesson ↔ ircserv A 層 Phase 対応表 |
| [poll_check.c](poll_check.c) | `POLLIN` 等のビットフラグ確認用（Lesson 2 前の実験） |

---

## Git タグ入門（メタ学習）

bircd カリキュラムでは **Lesson ごとに Git タグ** を使う。  
...
learn/bircd-curriculum ブランチ（先端は右へ伸びる）
    ...
    ●───●───●───●
    │       │
 lesson-1  lesson-2   ← タグはここに固定。Lesson 3 を足しても動かない
```

| 概念 | 例 | 動く？ | 用途 |
|------|-----|--------|------|
| **コミット SHA** | `bbc74b9` | 動かない | コミットそのもの。覚えにくい |
| **タグ** | `lesson-1` | 動かない | SHA への **人間が読める別名** |
...
| タグ | 指す Lesson | bircd の状態 |
|------|-----------|-------------|
| `lesson-1` | Lesson 1 完了 | select 版 + 教材（コード変更なし） |
| `lesson-2` | Lesson 2 完了 | poll 版 |
| `lesson-3` 以降 | （未作成） | ブランチに追加されたらタグも増える |
...
# Lesson 1 のコード全体に移動（読むだけ）
git checkout lesson-1

# Lesson 1 から自分用ブランチを切って Lesson 2 を自力実装
git checkout -b my-lesson2-work lesson-1

# Lesson 1 → Lesson 2 の差分だけ見る（最重要）
git diff lesson-1 lesson-2 -- bircd/

# 特定ファイルだけ Lesson 1 版を表示
git show lesson-1:bircd/init_fd.c

# ブランチ先端（最新 Lesson）に戻る
...
git fetch origin tag lesson-1 tag lesson-2
# または全タグ
...
git push origin lesson-1 lesson-2
```

### detached HEAD とは（注意）

`git checkout lesson-1` すると **detached HEAD** 状態になる。

- **できること:** 読む、ビルドする、`git diff` する
- **危ないこと:** そのまま編集してコミット → 枝のないコミットが迷子になる

**自分で Lesson 2 を書くとき** は必ず:
...
2. `git checkout lesson-1` と `git checkout learn/bircd-curriculum` の bircd/ の違いは？
3. `git diff lesson-1 lesson-2 -- bircd/` は何を見せる？
4. なぜ `git push` だけではタグが共有されないことがある？
5. Lesson 2 を自力実装するとき、なぜ `git checkout -b my-work lesson-1` が安全？

---

## 学習ブランチの使い方

学習用の変更は **`learn/bircd-curriculum` ブランチ1本** に Lesson ごとのコミット + **タグ** で管理する。
...
**B) Lesson 2 を自力で書く（推奨）**

```bash
git checkout -b my-lesson2-work lesson-1
# lesson2_guide.md に従って実装 …
git diff lesson-1 -- bircd/          # 自分の変更確認
git diff my-lesson2-work lesson-2 -- bircd/   # 模範解答との差分
```

**C) Lesson 2 完成版をそのまま動かす**
...
```
Lesson 1  bircd 完全理解（select 版）     … 必修          [tag: lesson-1]
Lesson 2  select → poll 変換              … [lesson2_guide.md](lesson2_guide.md)  [tag: lesson-2]
Lesson 3  受信・送信バッファリング         … 未着手
Lesson 4  ノンブロッキング I/O             … 未着手
Lesson 5  C++98 化（A 層 Server）          … ft_irc 本体（bircd 外）
```

| Lesson | bircd の状態 | 学ぶこと |
|-------|-------------|----------|
...
## Lesson 2 へ進む前のチェックリスト

Lesson 1 完了の目安（[bircd_learning_curriculum.md](bircd_learning_curriculum.md) 参照）:
...
問題なければ → **[lesson2_guide.md](lesson2_guide.md)** を開く。
...
Lesson 2 完了後は `src/a/Server.cpp`（または `a_tmp/Server.cpp`）を読み、「固定配列が vector になっただけ」と捉えると理解が早いです。
...
1. [bircd_learning_curriculum_ans.md](bircd_learning_curriculum_ans.md) の Lesson 2 セクション
2. `man 2 poll`
3. `git diff lesson-1 lesson-2 -- bircd/` で模範 diff を確認
4. チーム内で質問

---


## 教材管理者向け: 旧タグ `phase-*` から `lesson-*` への移行

ドキュメント表記を Phase → Lesson に統一したあと、**Git タグ名も手動で付け替える**（AI / 自動スクリプトでの git 操作は行わない）。

### 手順（ローカルで実行者が実行）

```bash
# 1. 旧タグが指すコミットに、新しい annotated タグを付ける
#    ^{ } はタグが指すコミット本体を取得（annotated tag 用）
git tag -a lesson-1 -m "Lesson 1: select 版 bircd + 教材" phase-1^{}
git tag -a lesson-2 -m "Lesson 2: select → poll 変換完了" phase-2^{}

# 2. 新タグの確認（旧タグと同じ SHA か）
git rev-parse phase-1^{} lesson-1^{}
git rev-parse phase-2^{} lesson-2^{}

# 3. リモートへ新タグを push
git push origin lesson-1 lesson-2

# 4. チーム合意後、旧タグを削除
git tag -d phase-1 phase-2
git push origin --delete phase-1 phase-2
```

### 注意

- リモートに `phase-*` が既に共有されている場合、**削除前に全員へ周知**する
- 旧タグを残したまま新タグだけ追加する運用も可（移行期間用）
- コミットメッセージ内の `Phase 2` 等は履歴のため**書き換えない**

---

## 教材管理者向け: タグの付け方

Lesson コミットを追加したあと、**そのコミットにタグを付けて push** する。

```bash
# Lesson 1 完了コミット（select 版 + 教材）にタグ
git tag -a lesson-1 -m "Lesson 1: select 版 bircd + 教材"

# Lesson 2 完了コミット（poll 版）にタグ
git tag -a lesson-2 -m "Lesson 2: select → poll 変換完了"
